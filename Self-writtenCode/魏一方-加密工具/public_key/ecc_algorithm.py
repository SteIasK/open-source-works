...
class ECC160:
    ...
    def __init__(self, private_key: str = None, public_key: str = None):
        try:
            ...
            raise RuntimeError("ECC key initialization error occurred") from e  
    ...
    def sign(self, message: str) -> bytes:
        ...
        """
        if not message:
            raise ValueError("Message is empty")  # 如果消息为空，抛出异常
        try:
            h = SHA256.new(message.encode('utf-8'))  # 计算消息的 SHA256 哈希值
            signer = DSS.new(self.key, 'fips-186-3')  
            return signer.sign(h)  
        except Exception as e:
            raise RuntimeError("ECC signing error occurred") from e  
